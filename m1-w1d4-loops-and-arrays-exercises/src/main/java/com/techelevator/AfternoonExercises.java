...
public class AfternoonExercises {
	...
	public boolean commonEnd(int[] a, int[] b) {
		...
		if (a[0] == b[0] || a[lastA - 1] == b[lastB - 1]) {
			return true;
		} else {
			return false;
		}
	...
	public int sum2(int[] nums) {
		...
		for ( int i = 0; i < nums.length; i++) {
			sumArray[i] = spotA + spotB;
		} 
	} else if (sumArray.length < 2);
		return spotA + spotB;
	...
	public int countEvens(int[] nums) {
		...
		int spotB = nums[1];
		int spotC = nums[2];
		int sumOfSpots = spotA + spotB + spotC;
		for ( int i = 0; i < nums.length; i++) {
			evenArray[i] = nums.length % 2;
		} 
		return sumOfSpots;
		}
	...
	public int sum13(int[] nums) {
		int[] numbersInArray = new int[nums.length];
		int sum = 0;
		if (nums.length == 13) {
			return 0;
	...
	public boolean has22(int[] nums) {
		int[] doesItHaveTwoTwos = new int[nums.length];
		if ( nums.length == 2) {
